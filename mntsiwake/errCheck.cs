...
namespace mntsiwake
{
    class errCheck
    {
        ...
        /// 結合枚数のチェック
        /// </summary>
        /// <param name="iX">伝票配列データの添え字</param>
                ...
                if (st[iX].Gyou[i].Torikeshi == "0")
                {
                    ...
                    if (st[iX].Gyou[i].ProjectCode.Trim() != string.Empty)
                    {
                            ...
                            st[iX].Gyou[i].Kashi.Kamoku == string.Empty && st[iX].Gyou[i].Kashi.Kin == string.Empty)
                        {
                            //エラーテーブルに値を格納
                            ErrCnt++;
                            ErrorTableSet(iX, i, "プロジェクト", st[iX].Gyou[i].ProjectCode, "勘定科目または金額が入力されていません。", MultiRow.DP_CODEP);
                        }
        ...
        public void ChkTekiyou(int iX, Entity.InputRecord[] st)
        {
			...
			{
                //摘要は１明細全角40、半角80がMAX
                if (System.Text.Encoding.GetEncoding("Shift_JIS").GetByteCount(st[iX].Gyou[i].Tekiyou) > 80)
                {
                    //エラーテーブルに値を格納
                    ErrCnt++;
        ...
        ///----------------------------------------------------------------
        /// <summary>
        ///     プロジェクトコードチェック </summary>
        /// <param name="sProject">
        ///     プロジェクトコード</param>
        /// <returns>
        ///     ok:true,NG:false</returns>
        ///----------------------------------------------------------------
        private Boolean ChkProjectIndi(string sProject)
        {
            Boolean wrkRetValue;
            string CodeB;

            //プロジェクトなしのときはOK
            if (sProject == string.Empty) return true;

            //プロジェクト登録が無しで、プロジェクト記入がある時NG
            if (global.pblProjectFlg == false && sProject != string.Empty) return false;

            //数字以外はOK、11桁以上は×
            if (sProject.Length > global.LEN_PROJECT) return false;

            //プロジェクトコード編集
            if (utility.NumericCheck(sProject))
            {
                CodeB = string.Format("{0:D20}", int.Parse(sProject));
            }
            else
            {
                CodeB = sProject.PadRight(20);
            }
            ...
            mySql += "SELECT ProjectCode,ProjectName,ValidDate,InValidDate  from tbProject ";
            ...
            while (dR.Read())
            {
                global._ValidDate = DateTime.Parse(dR["ValidDate"].ToString());
                global._InValidDate = DateTime.Parse(dR["InValidDate"].ToString());
            }
        ...
        ///-------------------------------------------------------------
        /// <summary>
        ///     プロジェクト有効期限チェック </summary>
        /// <param name="dDate">
        ///     伝票日付</param>
        /// <returns>
        ///     ok:true,NG:false</returns>
        ///-------------------------------------------------------------
        private Boolean ChkProjectDateIndi(string Year, string Month, string Day, String Hosei)
        {
            int wrkADYear;

            //伝票日付空欄のときエラーチェックせず終了
            if ((Year == string.Empty) || (Month == string.Empty) || (Day == string.Empty)) return true;

            //数字以外、2桁以上のときエラーチェックせず終了
            if (utility.NumericCheck(Year) == false || Year.Length > 2 ||
                utility.NumericCheck(Month) == false || Month.Length > 2 ||
                utility.NumericCheck(Day) == false || Day.Length > 2) return true;
            ...
            //日付変換可能か？
            DateTime r = new DateTime();
            if (DateTime.TryParse(wrkADYear.ToString() + "/" + Month + "/" + Day, out r) == false) return true;

            //伝票日付が有効期限内かチェックする
            if (r < global._ValidDate || r > global._InValidDate) return false;
        ...
        ///--------------------------------------------------------------------
        /// <summary>
        ///     サブプロジェクトコードチェック </summary>
        /// <param name="sProject">
        ///     サブプロジェクトコード</param>
        /// <returns>
        ///     ok:true,NG:false</returns>
        ///--------------------------------------------------------------------
        private Boolean ChkSubProjectIndi(string sProject)
        {
            Boolean wrkRetValue;
            string CodeB;
            ...
            //プロジェクト登録が無しで、プロジェクト記入がある時NG
            if (global.pblSubProjectFlg == false && sProject != string.Empty) return false;

            //数字以外はOK、2桁以上は×
            if (sProject.Length > global.LEN_SUBPROJECT) return false;
            ...
            mySql += "SELECT SubProjectCode,SubProjectName,ValidDate,InValidDate from tbSubProject ";
        ...
        ///--------------------------------------------------------------------
        /// <summary>
        ///     サブプロジェクト有効期限チェック </summary>
        /// <param name="dDate">
        ///     伝票日付</param>
        /// <returns>
        ///     ok:true,NG:false</returns>
        ///--------------------------------------------------------------------
        private Boolean ChkSubProjectDateIndi(string Year, string Month, string Day, String Hosei)
        {
            int wrkADYear;
