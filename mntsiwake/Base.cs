...
namespace mntsiwake
{
    public partial class Base : Form
    {
        ...
        private void LoadMaster()
        {
            ...
            //プロジェクトコード
            frmP.Text = "プロジェクトコードをロード中";
            frmP.progressValue = 90;
            frmP.ProgressStep();

            GridViewSetting_Project(fgProject); //グリッドビュー設定
            GridViewShow_Project(fgProject);    //グリッドにデータ表示

            ////////サブプロジェクトコード
            //////frmP.Text = "サブプロジェクトコードをロード中";
            //////frmP.progressValue = 99;
            //////frmP.ProgressStep();

            //////GridViewSetting_SubProject(fgSubProject);   //グリッドビュー設定
            //////GridViewShow_SubProject(fgSubProject);      //グリッドにデータ表示
        ...
        {
            //勘定奉行データベース接続文字列を取得する 2016/10/01
        ...
        /// <summary>
        /// プロジェクトコードグリッドビューの定義を行います
        /// </summary>
        /// <param name="tempDGV">プロジェクトコードグリッドビューオブジェクト</param>
        private void GridViewSetting_Project(DataGridView tempDGV)
        {
            try
            {
                //フォームサイズ定義
                ...
                // 各列幅指定
                tempDGV.Columns.Add("col1", "コード");
                tempDGV.Columns.Add("col2", "プロジェクト名");
                tempDGV.Columns.Add("col3", "有効開始");
                tempDGV.Columns.Add("col4", "有効終了");
        ...
        ///----------------------------------------------------------------------
        /// <summary>
        ///     グリッドビューへプロジェクトを表示する </summary>
        /// <param name="tempDGV">
        ///     DataGridViewオブジェクト名</param>
        ///----------------------------------------------------------------------
        private void GridViewShow_Project(DataGridView tempDGV)
        {
            string sDate;
            string sSonota = string.Empty;
            ...
            try
            {
                ...
                while (dR.Read())
                {
                    if (dR["ProjectCode"].ToString().Trim() != "00000000000000000000")
                    {
                        tempDGV.Rows.Add();

                        //コード
                        if (dR["ProjectCode"].ToString().Trim().Length > global.LEN_PROJECT)
                        {
                            tempDGV[0, iX].Value = dR["ProjectCode"].ToString().Trim().Substring(dR["ProjectCode"].ToString().Trim().Length - global.LEN_PROJECT, global.LEN_PROJECT);
                        }
                        else
                        {
                            tempDGV[0, iX].Value = dR["ProjectCode"].ToString().Trim();
                        }

                        tempDGV[1, iX].Value = dR["ProjectName"].ToString().Trim();     //名称

                        //有効期限開始
                        sDate = DateTime.Parse(dR["ValidDate"].ToString()).ToShortDateString();
                        if (sDate == "1753/01/02")
                        {
                            tempDGV[2, iX].Value = string.Empty;
                        }
                        else
                        {
                            tempDGV[2, iX].Value = sDate;
                        }

                        //有効期限終了
                        sDate = DateTime.Parse(dR["InValidDate"].ToString()).ToShortDateString();
                        if (sDate == "9999/12/31")
                        {
                            tempDGV[3, iX].Value = string.Empty;
                        }
                        else
                        {
                            tempDGV[3, iX].Value = sDate;
                        }

                        if (global.pblProjectFlg == false) global.pblProjectFlg = true;
                        iX++;
                    }
                    else
                    {
                        sSonota = dR["ProjectName"].ToString().Trim(); 
                    }
                }
                ...
                //プロジェクトデータありなら最終行に「その他」追加
                if (global.pblProjectFlg == true)
                {
                    tempDGV.Rows.Add();
                    tempDGV[0, iX].Value = "0";             //コード
                    tempDGV[1, iX].Value = sSonota;         //名称
                    tempDGV[2, iX].Value = string.Empty;    //有効期限開始
                    tempDGV[3, iX].Value = string.Empty;    //有効期限終了
                }
        ...
        /// <summary>
        /// サブプロジェクトコードグリッドビューの定義を行います
        /// </summary>
        /// <param name="tempDGV">サブプロジェクトコードグリッドビューオブジェクト</param>
        private void GridViewSetting_SubProject(DataGridView tempDGV)
        {
            try
            {
                //フォームサイズ定義
                ...
                // 各列幅指定
                tempDGV.Columns.Add("col1", "コード");
                tempDGV.Columns.Add("col2", "サブプロジェクト名");
                tempDGV.Columns.Add("col3", "有効開始");
                tempDGV.Columns.Add("col4", "有効終了");
        ...
        ///---------------------------------------------------------------
        /// <summary>
        ///     グリッドビューへサブプロジェクトを表示する </summary>
        /// <param name="tempDGV">
        ///     DataGridViewオブジェクト名</param>
        /// --------------------------------------------------------------
        private void GridViewShow_SubProject(DataGridView tempDGV)
        {
            string sDate;
            string sSonota = string.Empty;
            ...
            SqlDataReader dR;
            string sqlSTRING = string.Empty;
            sqlSTRING += "SELECT SubProjectCode,SubProjectName,ValidDate,InValidDate ";
            sqlSTRING += "from tbSubProject ";
            sqlSTRING += "ORDER BY SubProjectCode";

            dR = dCon.free_dsReader(sqlSTRING);

            try
            {
                global.pblSubProjectFlg = false;
                int iX = 0;
                ...
                while (dR.Read())
                {
                    if (dR["SubProjectCode"].ToString().Trim() != "00000000000000000000")
                    {
                        tempDGV.Rows.Add();

                        //コード
                        if (dR["SubProjectCode"].ToString().Trim().Length > global.LEN_KAMOKU)
                        {
                            tempDGV[0, iX].Value = dR["SubProjectCode"].ToString().Trim().Substring(dR["SubProjectCode"].ToString().Trim().Length - global.LEN_SUBPROJECT, global.LEN_SUBPROJECT);
                        }
                        else
                        {
                            tempDGV[0, iX].Value = dR["SubProjectCode"].ToString().Trim();
                        }

                        tempDGV[1, iX].Value = dR["SubProjectName"].ToString().Trim();     //名称
                        ...
                        if (global.pblSubProjectFlg == false) global.pblSubProjectFlg = true;
                        iX++;
                    }
                    else
                    {
                        sSonota = dR["SubProjectName"].ToString().Trim();
                    }
                }
                ...
                //サブプロジェクトデータありなら最終行に「その他」追加
                if (global.pblSubProjectFlg == true)
                {
                    tempDGV.Rows.Add();
                    tempDGV[0, iX].Value = "0";             //コード
                    tempDGV[1, iX].Value = sSonota;         //名称
        ...
        private void DataGetGyou(int iX, string readbuf, Entity.InputRecord[] sDenpyo)
        {
            ...
            //プロジェクトコード
            if (stData[16].Trim() != string.Empty)
            {
                sDenpyo[iX].Gyou[i].ProjectCode = stData[16].Trim().PadLeft(8, '0') + "-" + stData[17].Trim().PadLeft(2, '0');
            }
            else
            {
                sDenpyo[iX].Gyou[i].ProjectCode = string.Empty;
        ...
        /// エラーチェックルーチン
        /// </summary>
        /// <param name="dData">伝票配列データ</param>
        /// <param name="x1">エラーなしで処理を終了するとき：true、エラー有りまたはエラーなしで処理を終了しないとき：false</param>
        /// <returns>エラー配列データ</returns>
                ...
                for (int LineCnt = 0; LineCnt < global.MAXGYOU; LineCnt++)
                {
                    for (int j = 0; j < err.Length; j++)
                    {
                        if (err[j].DenNo == pCnt && err[j].LINE == LineCnt)
                        {
                            ...
                            if (err[j].Field != "借" && err[j].Field != "貸" && err[j].Field != "プロジェクト")
                            {
        ...
        private void KinCellValueChange(GrapeCity.Win.MultiRow.CellEventArgs e)
        {
            if (gcMultiRow1.GetValue(e.RowIndex, e.CellName) != null)
            {
                if (errCheck.ChkKinIndi(gcMultiRow1.GetValue(e.RowIndex, e.CellName).ToString()))
                {
                    if (gcMultiRow1.GetValue(e.RowIndex, e.CellName).ToString() != string.Empty)
                    {
                        gcMultiRow1.SetValue(e.RowIndex, e.CellName, string.Format("{0:#,###}", double.Parse(gcMultiRow1.GetValue(e.RowIndex, e.CellName).ToString().Replace(",", string.Empty))));
                    }
                }
            }
            else
        ...
        private void DlgDataGet()
        {
            ...
            for (int i = 0; i < global.MAXGYOU; i++)
			{
                ...
                //プロジェクトコード
                DenData[DenIndex].Gyou[i].ProjectCode = gcMultiRow1.GetValue(i, MultiRow.DP_CODEP).ToString();
                //DenData[DenIndex].Gyou[i].SubProjectCode = gcMultiRow1.GetValue(i, MultiRow.DP_CODESP).ToString();
			}
        }
        ...
        private void DenDataShift(int iX)
        {
            ...
            for (sCnt = iX; sCnt < DenData.Length - 1; sCnt++)
			{
                ...
                for (int i = 0; i < global.MAXGYOU; i++)
			    {
                    ...
                    DenData[sCnt].Gyou[i].ProjectCode = DenData[sCnt + 1].Gyou[i].ProjectCode;      // プロジェクトコード　2013/05/21
			    }
			}
